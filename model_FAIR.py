...
F2x = 3.74 #focring due to doubling CO2 (W/m2)
...
# make a df to hold the CO2 concentration with time and the radiative forcing
df = pd.DataFrame(index = timesteps, columns=['T_sfc','T_deep','C','F'], data = np.zeros((len(timesteps), 4)))
# set atmopsheric [CO2] for time t=0 (ppm)
df.iloc[0]['C'] = C0
...
for t in range(len(timesteps)-1):
	...
	df.iloc[t+1]['C'] = C0 + np.sum(R.iloc[t+1])
	...
	# step 5: global mean T anomaly
	df.iloc[t+1]['T_sfc'] = df.iloc[t]['T_sfc'] + (dt*yeartosec/(rho*cw*hsfc)) * (lb*df.iloc[t]['T_sfc'] + F + beta*e*(df.iloc[t]['T_deep'] - df.iloc[t]['T_sfc'])) 
	df.iloc[t+1]['T_deep'] = df.iloc[t]['T_deep'] + (dt*yeartosec/(rho*cw*hdeep)) * (beta * (df.iloc[t]['T_sfc'] - df.iloc[t]['T_deep'])) 

	# set the new alpha as alpha0
	alpha0 = alpha 
